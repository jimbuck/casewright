...
window.CW = (function () {
  ...
  // ---- recent repositories (launcher) ----
  const recents = [
    { name: "qa-testcases", path: "~/work/acme/qa-testcases", branch: "main",
      remote: "ssh://git@ssh.dev.azure.com/v3/acme/QA/qa-testcases",
      lastOpened: "2 minutes ago", workspaces: 4, lastWorkspace: "Payments QA", current: true },
    { name: "platform-wiki", path: "~/work/acme/platform.wiki", branch: "wikiMain",
      remote: "https://dev.azure.com/acme/Platform/_git/platform.wiki",
      lastOpened: "yesterday", workspaces: 7, lastWorkspace: "Checkout" },
    { name: "mobile-qa", path: "~/work/acme/mobile-qa", branch: "main",
      remote: "ssh://git@ssh.dev.azure.com/v3/acme/Mobile/mobile-qa",
      lastOpened: "3 days ago", workspaces: 2, lastWorkspace: "iOS" },
    { name: "onboarding-cases", path: "~/sandbox/onboarding-cases", branch: "main",
      remote: "(local only)", lastOpened: "last week", workspaces: 1, lastWorkspace: null },
  ];

  // ---- workspaces declared by casewright.json ----
  const rootConfig = { workspaces: ["areas/*", "qa/regression"] };
  const workspaces = [
    { id: "payments",  name: "Payments QA",  path: "areas/payments",
      description: "Manual test cases for the billing and payments area.",
      prefix: "PAY", runsDir: "runs" },
    { id: "onboarding", name: "Onboarding",  path: "areas/onboarding",
      description: "Sign-up, activation and first-run flows.",
      prefix: "ONB", runsDir: "runs" },
    { id: "regression", name: "Regression",  path: "qa/regression",
      description: "Cross-area regression suite.",
      prefix: "REG", runsDir: "runs" },
  ];

  // ---- helper to build a case ----
  let n = 0;
  const C = (o) => ({
    id: o.id, displayId: o.displayId, title: o.title,
    status: o.status || "active", tags: o.tags || [],
    suite: o.suite,
    objective: o.objective || "",
    systems: o.systems || [],
    steps: o.steps || [],         // {text, depth}
    expected: o.expected || [],
    modified: false,
  });

  const cases = [
    C({ id: "9f3a7c1e8b", displayId: "PAY-0042", suite: "auth",
      title: "User can reset password from the login screen",
      status: "active", tags: ["Auth", "Smoke", "Regression"],
      objective: "Verify a registered user can reset their password and regain access via the **self-service** flow; no admin involvement required.",
    ...
    C({ id: "c4d8e012af", displayId: "PAY-0043", suite: "auth",
      title: "Account locks after five failed sign-in attempts",
      status: "active", tags: ["Auth", "Security", "Regression"],
      objective: "Confirm the lockout policy engages after repeated failures and communicates a clear recovery path to the user.",
    ...
    C({ id: "7b22aa90d1", displayId: "PAY-0051", suite: "sessions",
      title: "Idle session expires and prompts re-authentication",
      status: "active", tags: ["Auth", "Sessions"],
      objective: "An idle session must expire on schedule and require re-authentication without losing unsaved draft data.",
    ...
    C({ id: "e019b7c33a", displayId: "PAY-0088", suite: "billing",
      title: "Coupon applies a percentage discount at checkout",
      status: "active", tags: ["Billing", "Smoke", "Regression"],
      objective: "A valid percentage coupon should reduce the order total correctly and reflect in the receipt.",
    ...
    C({ id: "a8f1029ce5", displayId: "PAY-0090", suite: "billing",
      title: "Declined card surfaces an actionable error",
      status: "draft", tags: ["Billing", "Payments"],
      objective: "When a card is declined the user must see a clear, recoverable error and the order must not be created.",
      ...
      expected: [
        "A clear decline message is shown.",
        "No order or charge is created.",
      ] }),
    C({ id: "12be77a4c0", displayId: "PAY-0091", suite: "billing",
      title: "Refund returns funds to the original payment method",
      status: "active", tags: ["Billing", "Refunds", "Regression"],
      objective: "A refund issued from the admin should return funds to the original method and update the ledger and receipt.",
    ...
    C({ id: "5d6e9981bb", displayId: "PAY-0102", suite: "subs",
      title: "Plan upgrade prorates the remaining period",
      status: "active", tags: ["Billing", "Subscriptions", "Regression"],
      objective: "Upgrading mid-cycle must charge a prorated amount for the remainder of the billing period.",
    ...
    C({ id: "f7710c2ad9", displayId: "PAY-0103", suite: "subs",
      title: "Cancellation takes effect at period end",
      status: "deprecated", tags: ["Subscriptions"],
      objective: "A standard cancellation should keep access until the end of the paid period, then stop renewal.",
      ...
      expected: [
        "Access remains until the period end date.",
        "No further renewal is charged.",
      ] }),
  ];

  // ---- suite tree (folders) for the Payments workspace ----
  const tree = [
    { type: "suite", id: "auth", name: "Authentication", path: "Authentication",
      children: [
        { type: "case", id: "9f3a7c1e8b" },
        { type: "case", id: "c4d8e012af" },
        { type: "suite", id: "sessions", name: "Sessions", path: "Authentication/Sessions",
          children: [ { type: "case", id: "7b22aa90d1" } ] },
      ] },
    { type: "suite", id: "billing", name: "Billing", path: "Billing",
      children: [
        { type: "case", id: "e019b7c33a" },
        { type: "case", id: "a8f1029ce5" },
        { type: "case", id: "12be77a4c0" },
      ] },
    { type: "suite", id: "subs", name: "Subscriptions", path: "Subscriptions",
      children: [
  ...
  // ---- runs (CSV-backed) ----
  const runs = [
    { id: "run-sprint12", name: "Regression — Sprint 12",
      file: "runs/2026-06-01-regression-sprint12.csv",
      created: "2026-06-01", status: "open", scope: "tag: regression",
      rows: [
    ...
    { id: "run-smoke-0518", name: "Smoke — pre-release",
      file: "runs/2026-05-18-smoke.csv",
      created: "2026-05-18", status: "closed", scope: "tag: smoke",
      rows: [
  ...
  // ============================================================
  // Conflict scenario — produced by a Pull.
  // Mix of auto-merged elements (one-sided) and true conflicts
  // (changed on both sides). Drives the structured merge resolver.
  // ============================================================
  const conflict = {
    branch: "main",
    behind: 3, ahead: 1,
    files: [
      {
        kind: "case", path: "areas/payments/Authentication/reset-password.md",
        displayId: "PAY-0042", caseId: "9f3a7c1e8b",
        title: "User can reset password from the login screen",
        elements: [
          { key: "title", label: "Title", kind: "field",
            ...
            auto: "theirs", reason: "Changed only on incoming (theirs)." },
          { key: "status", label: "Status", kind: "field",
            base: "active", ours: "active", theirs: "active", auto: "same" },
          { key: "tags", label: "Tags", kind: "tags",
            base: ["Auth", "Smoke", "Regression"],
            ours: ["Auth", "Smoke", "Regression", "P1"],
            theirs: ["Auth", "Smoke", "Regression", "Email"],
            auto: "merge", merged: ["Auth", "Smoke", "Regression", "P1", "Email"],
            reason: "Set merge: both additions kept (P1 + Email)." },
          { key: "objective", label: "Objective", kind: "prose", conflict: true,
          ...
          { key: "systems", label: "Systems in Scope", kind: "list",
            ...
            auto: "theirs", reason: "Changed only on incoming (added Notification service)." },
          { key: "steps", label: "Steps", kind: "steps", conflict: true,
            base: [
              { text: "Navigate to the login screen.", depth: 0 },
              { text: "Click \"Forgot password\".", depth: 0 },
              { text: "Enter the account email and submit.", depth: 0 },
            ...
            reason: "Changed on both sides — choose one or edit." },
          { key: "expected", label: "Expected Results", kind: "list",
      ...
      {
        kind: "case", path: "areas/payments/Billing/coupon-discount.md",
        displayId: "PAY-0088", caseId: "e019b7c33a",
        title: "Coupon applies a percentage discount at checkout",
        elements: [
          { key: "title", label: "Title", kind: "field",
            base: "Coupon applies a percentage discount at checkout",
            ours: "Coupon applies a percentage discount at checkout",
            theirs: "Coupon applies a percentage discount at checkout", auto: "same" },
          { key: "displayId", label: "Display ID", kind: "field",
            base: "PAY-0088", ours: "PAY-0088", theirs: "PAY-0088", auto: "same" },
          { key: "status", label: "Status", kind: "field",
            base: "draft", ours: "active", theirs: "draft",
            auto: "ours", reason: "Changed only locally (ours) to active." },
          { key: "expected", label: "Expected Results", kind: "list", conflict: true,
      ...
      {
        kind: "run", path: "areas/payments/runs/2026-06-01-regression-sprint12.csv",
        title: "Regression — Sprint 12",
        rows: [
          { case_id: "9f3a7c1e8b", display_id: "PAY-0042", auto: "same",
            value: { result: "pass", tester: "amartin", notes: "" } },
          { case_id: "c4d8e012af", display_id: "PAY-0043", auto: "theirs",
            reason: "Result recorded only on incoming.",
            value: { result: "pass", tester: "okeefe", notes: "" } },
          { case_id: "e019b7c33a", display_id: "PAY-0088", conflict: true,
  ...
  return { recents, rootConfig, workspaces, cases, tree, runs, conflict };
})();
